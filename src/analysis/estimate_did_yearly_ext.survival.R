...
quartiles <- dtycj[, quantile(Scaler1, probs = seq(0, 1, 0.25), na.rm = TRUE)]
dtycj[, sizeQuartile := cut(Scaler1, breaks = quartiles, labels = 1:4)]
quartiles <- dtycj[, quantile(firm_age, probs = seq(0, 1, 0.25), na.rm = TRUE)]
dtycj[, ageQuartile := cut(firm_age, breaks = quartiles, labels = 1:4)]
