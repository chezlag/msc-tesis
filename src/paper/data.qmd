...
En este trabajo combino varias fuentes de datos de las firmas uruguayas del período 2009–2016. Uso principalmente microdatos de la Dirección General Impositiva (DGI) que contienen información sobre distintos tipos de actividad de las firmas. La @tbl-data resume las fuentes de datos y su período de cobertura.

En primer lugar, utilizo información derivada de declaraciones juradas que las firmas deben presentar periódicamente ante la autoridad tributaria. Cada declaración o formulario se asocia al pago de distintos impuestos. Por un lado, uso los formularios 2148 y 2149, vincluados a IRAE e IPAT, que contienen información patrimonial y de resultados de la firma. Ambos formularios contienen la misma información pero difieren en su frecuencia. El formulario 2148 tiene frecuencia anual y es obligatorio para todas las firmas. El formulario 2149 tiene frecuencia semestral y solo lo completa un subconjunto de firmas que se encuentra bajo escrutinio especial de DGI [^data-1].

[^data-1]: Hay aproximadamente 10.000 firmas sujetas al control especial de empresas (firmas CEDE, ver Apéndice B).
...
En tercer lugar, cuento con información del sistema de facturación electrónica. Dispongo de un registro de transacciones que vincula duplas de firmas, una emisora y una receptora. La información está agregada a nivel mensual, y registra el número de transacciones de la dupla cada mes, y el valor agregado de las mismas. A partir de esta base, puedo identificar el mes exacto en que cada firma comienza a estar expuesta a la facturación electrónica –ya como receptor o emisor–, así como la intensidad de su exposición.
...
Por otro lado, utilizo un conjunto de covariables pre-tratamiento en las estimaciones para controlar tendencias previas diferenciales. Defino cuartiles de facturación pre-tratamiento, cuartiles de activos pre-tratamiento, cuartiles de edad de las firmas, y nueve sectores de actividad.
...
La segunda razón es que sería deseable que las firmas tratadas no tuvieran un poder de influencia en la decisión de sus proveedores de comenzar a emitir e-facturas. Aunque la pretensión de exogeneidad pueda ser débil, el reducido tamaño de las firmas analizadas y su poca importancia como jugadores individuales, vuelve plausible que el momento exacto en que las firmas empiezan a recibir facturas electrónicas sea relativamente exógeno.
...
**Tratamiento absorbente.** La recepción de facturas electrónicas opera, en la mayoría de los casos, como un tratamiento absorbente. Es decir, una vez una firma recibe su primer comprobante electrónico, no deja de recibirlos todos los años hasta el final del período. Menos de 1% de las firmas en el panel balanceado recibe una factura electrónica un año y deja de recibirla en alguno de los años siguientes. Dada las restricciones de la metodología y que se trata de un porcentaje relativamente menor de mi muestra, excluyo a las firmas para las que el tratamiento no es absorbente de la muestra principal. Vale notar que en el universo de firmas la no-absorción del tratamiento supone un problema mayor. Para evaluar la política en el total de las firmas uruguayas sería necesario usar métodos alternativos que contemplen tratamientos no absorbentes [ver por ejemplo @dechaisemartinDifferenceinDifferencesEstimatorsIntertemporal2022].
