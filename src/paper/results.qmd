...
estname <- c("S1.bal.ctrl", "S2.bal.ctrl", "S3.unbal.ctrl")
...
ar <- data.frame(
  ...
  lapply(
    ...
    \(x) dty[year <= 2011 & G1 < 2016, fmean(get(x), na.rm = TRUE)]
  )
)
tbl %>%
...
tidy_size <- simple_size %>%
  ...
  reduce(rbind) %>%
  setDT() %>%
  .[, variable := str_remove_all(y.name, "^Scaled2|K$") %>%
    as_factor()]
...
Al realizar las estimaciones por sector de actividad, se observa que la recepción de e-facturas tiene efectos significativos únicamente en algunos sectores. En particular, en el comercio minorista y en la industria manufacturera se observa un aumento de los costos reportados de entre \$0.17-\$0.20 por peso de facturación rezagada. A su vez, la industria manufacturera es el único sector en el cual la recepción de e-facturas aumenta significativamente los ingresos reportados, con una estimación puntual de \$0.236 por peso de facturación rezagada. Por último, hay aumentos significativos del pago de IVA en el sector servicios y, nuevamente, las manufacturas. El pago de IVA aumenta en promedio \$0.01–\$0.03 por peso de facturación rezagada.
...
La @fig-ext.survival-dynamic muestra los efectos de tratamiento dinámicos para cada una de las variables mencionadas, para testear las tendencias paralelas antes del tratamiento. Ninguno de los coeficientes pre-tratamiento resulta significativo excepto el asociado a un año antes del tratamiento, lo cual podría estar indicando algún tipo de anticipación de las firmas. El método de @callawayDifferenceinDifferencesMultipleTime2021 contempla la posibilidad de que las firmas se anticipen al tratamiento, por lo que incluyo en todas las especificaciones de esta sección la posibilidad que las firmas se anticipen un período al tratamiento.
...
est_dyn_survival <-
  readRDS(awd(
    "out/analysis/did_yearly_ext.survival_S3.bal.base_aggte.dynamic.RDS"
  ))

est_survival <- 
  readRDS(awd(
    "out/analysis/did_yearly_ext.survival_S3.bal.base.RDS"
  ))
