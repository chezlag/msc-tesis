...
En esta sección presento evidencia sobre los efectos de la recepción de e-facturas en el margen extensivo. Estudio salidas de la muestra, cambios en la franja de renta ficta de IRAE, y pasajes a realizar declaraciones juradas reales de la actividad de la firma.
...
```{r fig-survival-dynamic}
est_dyn_survival <- readRDS(paste0(ROOT, "out/analysis/did_yearly_survival_S3.bal.base_aggte.dynamic.RDS"))
...
```{r tbl-survival-overall-att}
#| tbl-cap: Efecto de tratamiento agregado sobre probabilidad de supervivencia. Panel balanceado.
estlist <- readRDS(awd("out/analysis/did_yearly_survival_S3.bal.base_aggte.simple.RDS"))
tbl <- estlist %>%
  map(tidy_did_list) %>% 
  .[c(5, 1:4)]
colnames <- list.cases(tbl, tidy$y.name, sorted = FALSE)
