...
```{r fig-heterog-size-dynamic-1}
tidy_size <- readRDS(paste0(ROOT, "out/analysis/did_yearly_by.size_S1.bal.base_aggte.dynamic.RDS")) %>%
  map(possibly(tidy_did, NULL)) %>%
  reduce(rbind) %>%
  setDT() %>%
...
varlist <- c("deductPurchases", "taxableTurnover")
tidy_size[variable %in% varlist] %>%
  ggplot(aes(x = event, y = estimate, color = as_factor(sizeQuartile))) +
  ...
  labs(color = "Pre-treatment size quartiles", caption = "[ 95% Simult. CI ]")
```

```{r fig-heterog-size-dynamic-2}
varlist <- c("vatPurchases", "vatSales", "vatPaid")
tidy_size[variable %in% varlist] %>%
  ggplot(aes(x = event, y = estimate, color = as_factor(sizeQuartile))) +
