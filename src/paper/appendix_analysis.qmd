...
dts[, neverTreated := is.na(dateFirstReception)]
dty[, neverTreated := is.na(dateFirstReception)]
```
...
#| tbl-cap: Descriptivas de panel balanceado y no balanceado (1 obs. = 1 empresa).
...
dts[(inSample3), ..varlist] %>%
  tbl_summary(by = "inSample3", missing = "no")
```

```{r tbl-yearly-characteristics-desgranamiento}
#| tbl-cap: Descriptivas de panel balanceado y no balanceado (1 obs. = 1 año-empresa).
...
dty[inSample3 & turnoverMUI %between% c(0.01, 30)] |>
...
#| tbl-cap: Descriptivas de nunca tratados. Muestra principal. (1 obs. = 1 año-empresa).
...
dty[inSample3 & djFict & year %in% 2009:2015, ..varlist] %>%
  .[, Scaler1 := Scaler1 / 1e06] %>%
  tbl_summary(
    by = "neverTreated",
    # statistic = list(all_continuous() ~ "{mean} ({sd})"),
...
dty[, .N, eventtime]
dty[turnoverMUI < 5 & inSample3 & eventtime %in% c(-1, 1)] |>
...
tidy_size <- readRDS(paste0(ROOT, "out/analysis/did_yearly_by.size_S1.bal.ctrl_aggte.dynamic.RDS")) %>%
  map(possibly(tidy_did, NULL)) %>%
  reduce(rbind) %>%
  setDT() %>%
...
tidy_size[term != "ATT (l = -4)", sizeQuartile := nafill(as.integer(sizeQuartile), type = "locf")]
...
varlist <- c("imputedPurchases", "vatSales", "vatPaid")
...
var_regex <- "imputedPurchases|taxableTurnover|vatPurchases|vatSales|vatPaid"
ar <- data.frame(
  "Mean Y pre-2011",
  lapply(
