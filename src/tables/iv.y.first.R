library(groundhog)
pkgs <- c(
  "fastverse",
  "forcats",
  "gt",
  "magrittr",
  "modelsummary",
  "purrr",
  "stringr"
)
date <- "2024-01-15"
groundhog.library(pkgs, date)
source("src/lib/cli_parsing_o.R")

# Input -----------------------------------------------------------------------

est <- readRDS("out/analysis/iv.first.RDS")

# Table -----------------------------------------------------------------------

ylablist <- c("IHS (e-invoiced input VAT)", "IHS (N e-tickets received)")
rows <- data.table(
  `Firm & year FE` = rep("Y", length(est)),
  Balanced = rep(c("Y", "Y", ""), length(ylablist)),
  `Winsorized at p99` = rep(c("Y", "", "Y"), length(ylablist)),
  `Winsorized at p95` = rep(c("", "Y", ""), length(ylablist)),
  `Allowing exits` = rep(c("", "", "Y"), length(ylablist))
) |>
  t() %>%
  cbind(rownames(.), .) |>
  data.frame()
rownames(rows) <- NULL

gtbl <- est %>%
  msummary(
    gof_map = "nobs",
    output = "gt",
    stars = TRUE,
    add_rows = rows
  ) %>%
  text_replace(pattern = "IHSeticketTaxK$", "IHS (e-invoiced VAT)") |>
  text_replace(pattern = "IHSnTicketsReceived$", "IHS (N e-invoices)") |>
  text_replace(pattern = "^probTicketReception$", "Prob (e-invoice reception)") |>
  tab_spanner(label = ylablist[1], columns = 2:4) %>%
  tab_spanner(label = ylablist[2], columns = 5:7) %>%
  tab_style(
    style = cell_borders(
      sides = c("right"),
      style = "solid"
    ),
    locations = cells_body(
      columns = c(1, 4, 7)
    )
  ) |>
  opt_table_font(font = "Times New Roman")

opt$output <- "out/tables/iv.y.first.png"
gtsave(gtbl, opt$output)
opt$output <- "out/tables/iv.y.first.tex"
gtsave(gtbl, opt$output)

# Manually adjust tex table
tex <- readLines(opt$output)
tex <- append(tex, "\\midrule", after = grep("Num.Obs", tex) - 1)
pval_ref <- c(
  "\\midrule",
  "\\multicolumn{7}{l}{* p $<$ 0.05, ** p $<$ 0.01, *** p $<$ 0.001} \\\\"
)
tex <- append(tex, pval_ref, after = grep("bottomrule", tex) - 1)
tex <- map_chr(tex, \(x) str_replace(x, "longtable", "tabular"))
tex <- head(tex, length(tex) - 4)
tex[length(tex)] <- "\\end{tabular}%"
writeLines(tex, opt$output)
