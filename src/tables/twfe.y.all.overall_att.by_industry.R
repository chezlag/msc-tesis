...
est <- readRDS("out/analysis/twfe.y.all.tab5.RDS")
...
gtbl <- est %>%
  ...
  text_replace(pattern = "^treatBelowTRUE$", "Treated × Low HH cons. industry") %>%
  text_replace(pattern = "^treatAboveTRUE$", "Treated × High HH cons. industry") %>%
...
opt$output <- "out/tables/twfe.y.all.overall_att.by_industry.png"
gtsave(gtbl, opt$output)
opt$output <- "out/tables/twfe.y.all.overall_att.by_industry.tex"
gtsave(gtbl, opt$output)
