...
est <- readRDS("out/analysis/twfe.y.all.tab7.RDS")
...
gtbl <- est %>%
  ...
  text_replace(pattern = "^treatBelowTRUE$", "Treated × Low imports industry") %>%
  text_replace(pattern = "^treatAboveTRUE$", "Treated × High imports industry") %>%
...
opt$output <- "out/tables/twfe.y.all.overall_att.by_imports.png"
gtsave(gtbl, opt$output, zoom = 2)
opt$output <- "out/tables/twfe.y.all.overall_att.by_imports.tex"
