...
est <- readRDS("out/analysis/twfe.y.all.tab6.RDS")
...
gtbl <- est %>%
  ...
  text_replace(pattern = "^treatBelowTRUE$", "Treated × Low export industry") %>%
  text_replace(pattern = "^treatAboveTRUE$", "Treated × High export industry") %>%
...
opt$output <- "out/tables/twfe.y.all.overall_att.by_export.png"
gtsave(gtbl, opt$output)
opt$output <- "out/tables/twfe.y.all.overall_att.by_export.tex"
gtsave(gtbl, opt$output)
